...
keys.forEach(key => {
  ...
  o.frequency.setTargetAtTime(Math.floor(key.freq), context.currentTime, 0);
  o.connect(context.destination);
  
  const startKeyDiv = document.createElement('button');
  startKeyDiv.innerHTML = key.name + " >";
  startKeyDiv.addEventListener('click', () => o.start());
  
  const stopKeyDiv = document.createElement('button');
  stopKeyDiv.innerHTML = key.name + " ||";
  stopKeyDiv.addEventListener('click', () => o.stop());  
  
  document.body.appendChild(startKeyDiv);
  document.body.appendChild(stopKeyDiv);
})